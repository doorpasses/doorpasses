...
description: 'Guidelines for building robust integrations with DoorPasses API'
---
...
const client = new DoorPassesClient({
  accountId: process.env.DOORPASSES_ACCOUNT_ID,
  sharedSecret: process.env.DOORPASSES_SHARED_SECRET,
  baseUrl: 'https://api.doorpasses.com' // Always HTTPS
});
```
...
async function issueAndTrackPass(employeeData) {
  ...
  await db.employees.update({
    ...
    data: {
      doorpasses_pass_id: accessPass.data.id,
...
```javascript
app.post('/webhooks/doorpasses', async (req, res) => {
...
const client = new DoorPassesClient({
  accountId: environment === 'production'
    ? process.env.DOORPASSES_PROD_ACCOUNT_ID
    : process.env.DOORPASSES_SANDBOX_ACCOUNT_ID,
  sharedSecret: environment === 'production'
    ? process.env.DOORPASSES_PROD_SECRET
    : process.env.DOORPASSES_SANDBOX_SECRET,
  baseUrl: environment === 'production'
    ? 'https://api.doorpasses.com'
    : 'https://sandbox.doorpasses.com'
});
```
...
```javascript
describe('DoorPasses Integration', () => {
...
```javascript
// __mocks__/@doorpasses/node-sdk.js
export const DoorPassesClient = jest.fn().mockImplementation(() => ({
  accessPasses: {
...
const logger = winston.createLogger({
  ...
  transports: [
    new winston.transports.File({ filename: 'doorpasses.log' })
  ]
});
 ...
 * @throws {DoorPassesError} If API request fails
 *
 * @example
