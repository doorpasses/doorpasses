---
title: 'Examples'
description: 'Code examples for the DoorPasses Rust SDK'
---
...
```rust
use doorpasses::{DoorPasses, types::{IssueAccessPassParams, Classification}};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = DoorPasses::new(
        std::env::var("DOORPASSES_ACCOUNT_ID")?,
        std::env::var("DOORPASSES_SHARED_SECRET")?
    )?;
...
```rust
use doorpasses::{DoorPasses, types::IssueAccessPassParams, error::{DoorPassesError, Result}};
...
async fn issue_pass_with_retry(
    ...
    loop {
        ...
        match client.access_passes.issue(params.clone()).await {
            ...
            Err(DoorPassesError::RateLimitExceeded) if attempts < max_retries => {
                eprintln!("Rate limited, waiting before retry...");
                tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;
                continue;
            }
            Err(DoorPassesError::Timeout) if attempts < max_retries => {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=10 {
        let client_clone = DoorPasses::new(
            std::env::var("DOORPASSES_ACCOUNT_ID")?,
            std::env::var("DOORPASSES_SHARED_SECRET")?
        )?;
...
```rust
use doorpasses::{DoorPasses, types::{IssueAccessPassParams, AccessPass}, error::Result};
...
impl PassManager {
    pub fn new(client: DoorPasses) -> Self {
        Self { client }
    }
...
Find more examples in the [GitHub repository](https://github.com/mohammedzamakhan/doorpasses/tree/main/packages/rust-sdk/examples):
...
<CardGroup cols={2}>
  <Card title="API Reference" icon="book" href="https://docs.rs/doorpasses">
    View full API documentation
  </Card>
  <Card title="GitHub Repository" icon="github" href="https://github.com/mohammedzamakhan/doorpasses">
    View source code
  </Card>
</CardGroup>
