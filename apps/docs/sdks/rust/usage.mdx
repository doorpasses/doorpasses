---
title: 'Usage'
description: 'How to use the DoorPasses Rust SDK'
---

## Client Initialization

The DoorPasses client is the main entry point for interacting with the API.
...
.with_base_url("https://api.doorpasses.io".to_string())
...
```rust
use doorpasses::types::{IssueAccessPassParams, Classification};
...
```rust
use doorpasses::types::{ListAccessPassesParams, AccessPassState};
...
```rust
use doorpasses::types::UpdateAccessPassParams;
...
```rust
use doorpasses::types::UpdateCardTemplateParams;
...
match client.access_passes.issue(params).await {
    ...
    Err(DoorPassesError::RateLimitExceeded) => {
        eprintln!("Rate limit exceeded, please retry later");
    }
    Err(DoorPassesError::Timeout) => {
...
```rust
use doorpasses::types::{Platform, Protocol, UseCase, Classification, AccessPassState};
