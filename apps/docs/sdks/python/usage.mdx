---
title: 'Usage Guide'
description: 'How to use the DoorPasses Python SDK'
---

## Client Initialization

Initialize the DoorPasses client with your credentials:
...
```python
from doorpasses import DoorPassesClient, ValidationError, UnauthorizedError, DoorPassesError
...
```python
client = DoorPassesClient(
    account_id=os.environ['DOORPASSES_SANDBOX_ACCOUNT_ID'],
    shared_secret=os.environ['DOORPASSES_SANDBOX_SECRET'],
    base_url='https://sandbox.doorpasses.com'
)
```
...
```python
import asyncio
from doorpasses import AsyncDoorPassesClient

async def main():
    client = AsyncDoorPassesClient(
        account_id=os.environ['DOORPASSES_ACCOUNT_ID'],
        shared_secret=os.environ['DOORPASSES_SHARED_SECRET']
    )
...
```python
from doorpasses import DoorPassesClient
from doorpasses.types import CreateAccessPassRequest, AccessPass
...
<CardGroup cols={2}>
  ...
  <Card title="Async Operations" icon="clock">
    Use AsyncDoorPassesClient for high-throughput applications
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card
    title="GitHub"
    icon="github"
    href="https://github.com/doorpasses/python-sdk"
  >
