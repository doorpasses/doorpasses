...
<Steps>
  <Step title="Verify Headers">
    ...
    ```bash
    curl -v https://api.doorpasses.com/v1/access-passes \
  ...
  <Step title="Check Account ID">
    ...
    // Should match the ID provided by DoorPasses
    ```
  </Step>
...
3. **Test Install URL**:
   ```bash
   curl -I https://doorpasses.com/install/0xp4551d
...
<Tabs>
  ...
  <Tab title="Check Rate Limit Headers">
    ...
    const response = await fetch('https://api.doorpasses.com/v1/access-passes', {
      headers: {
        'X-ACCT-ID': accountId,
...
**Problem**: `Cannot find module '@doorpasses/node-sdk'`
...
// With types
import type { AccessPass, CreateAccessPassRequest } from '@doorpasses/node-sdk';
```
</CodeGroup>
...
<Check>No firewall blocking DoorPasses IPs</Check>
<Check>Webhook URL is correctly registered</Check>

**Testing**:
```bash
# Test your webhook endpoint manually
curl -X POST https://your-domain.com/webhooks/doorpasses \
...
**Solution**:
```javascript
app.post('/webhooks/doorpasses', (req, res) => {
  const authHeader = req.headers.authorization;
  const expectedToken = process.env.DOORPASSES_WEBHOOK_SECRET;
...
<Tabs>
  ...
  <Tab title="Python">
    ...
    client = DoorPassesClient(
        account_id=os.environ['DOORPASSES_ACCOUNT_ID'],
        shared_secret=os.environ['DOORPASSES_SHARED_SECRET'],
        debug=True
    )
    ```
  </Tab>
...
```javascript
const client = new DoorPassesClient({
  accountId: process.env.DOORPASSES_SANDBOX_ACCOUNT_ID,
  sharedSecret: process.env.DOORPASSES_SANDBOX_SECRET,
...
<CardGroup cols={2}>
  <Card title="Check API Status" icon="signal">
    Visit https://status.doorpasses.com for system status
  </Card>
  ...
  <Card title="Contact Support" icon="envelope">
    Email support@doorpasses.com with error details
  </Card>
...
- SDK: @doorpasses/node-sdk v1.2.3
