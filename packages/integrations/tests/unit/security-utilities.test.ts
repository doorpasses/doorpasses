...
class MockWebhookValidator {
	...
	static validateSignature(
		...
		return integrationEncryption.secureCompare(
			expectedSignature,
			providedSignature,
		)
	}
...
describe('MockWebhookValidator', () => {
	...
	describe('validateSignature', () => {
		it('should validate correct signature', () => {
			...
			const result = MockWebhookValidator.validateSignature(
				testPayload,
				signature,
				testSecret,
			)
	...
	describe('validateGitHubSignature', () => {
		it('should validate GitHub signature format', () => {
			...
			const result = MockWebhookValidator.validateGitHubSignature(
				testPayload,
				signature,
				testSecret,
			)
	...
	describe('validateSlackSignature', () => {
		it('should validate correct Slack signature', () => {
			...
			const signature =
				'v0=' +
				crypto.createHmac('sha256', testSecret).update(baseString).digest('hex')
...
describe('MockSecurityValidator', () => {
	...
	describe('validateOAuthState', () => {
		it('should validate correct OAuth state', () => {
			...
			const state = integrationEncryption.generateOAuthState(
				organizationId,
				providerName,
			)
	...
	describe('validateApiKeyFormat', () => {
		...
		it('should handle different API key formats', () => {
			...
			const result = MockSecurityValidator.validateApiKeyFormat(
				slackKey,
				slackFormat,
			)
			expect(result).toBe(true)
